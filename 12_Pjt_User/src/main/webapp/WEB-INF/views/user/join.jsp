...
<script>
	...
	function fnCheckId(){
	  $('#id').on('keyup', function(){
		  ...
		  if(verifyId == false){
			  ...
			  return;  // 여기서 함수 실행을 종료한다. (이후에 나오는 ajax(중복 체크) 실행을 막기 위해서. 정규식 검사 실패했는데 중복 체크를 할 필요가 없으니까)
		  } 
  ...
  function fnCheckEmail(){
	  ...
	  new Promise(function(resolve, reject){
		  ...
		  // 정규식 실패 메시지 + 이메일 중복체크 실패 메시지 -> 이 두 가지 동시에 뜨면 구분 못한다.
		  // 구분하는 방법 : 매개변수를 전달하면 된다. (1 , 2) -> catch(function())이 받는다.
