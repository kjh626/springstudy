	...
	$(function(){
		...
		$('#query').on('keyup', function(){
			...
			$.ajax({
				...
				//응답
				dataType: 'json',
				// 보낼 때는 E.FIRST_NAME: 받아올 떄는 firstName으로 온다.
				success:function(resData){    // resData = {"employees": [{"firstName": "xxx", "deptDTO":{"departmentName": "xxx"}}, {}, {}, ...]}
					$.each(resData.employees, function(i, employee){
						switch($('#column').val()){
						case "E.FIRST_NAME":
							$('#auto_complete').append('<option value="' + employee.firstName + '" />'); break;
						case "E.PHONE_NUMBER":
							$('#auto_complete').append('<option value="' + employee.phoneNumber + '" />'); break;
						case "D.DEPARTMENT_NAME":
							$('#auto_complete').append('<option value="' + employee.deptDTO.departmentName + '" />'); break;
						}
					})
				}
