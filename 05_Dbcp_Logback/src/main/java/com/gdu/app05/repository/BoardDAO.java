...
public class BoardDAO {
	...
	// BoardDAO의 생성자 (webapp/META_INF/context.xml에 작성한 <Resource> 태그 읽기)
	public BoardDAO() {
		// JNDI 방식 : <Resource> 태그의 name 속성으로 Resource를 읽어들이는 방식
		try {
			Context context = new InitialContext();
			dataSource = (DataSource) context.lookup("java:comp/env/jdbc/GDJ61"); // 오브젝트로 반환해서 캐스팅 해줘야 한다.
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void close() {
		try {
			if(rs != null) rs.close();
			if(ps != null) ps.close();
			if(con != null) con.close();  // 사용한 Connection을 dataSource에게 반납한다.(대여와 반납의 개념으로 Connection이 사용됨)
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
